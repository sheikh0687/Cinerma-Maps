...
struct Api_ToursimChildCategory : Codable {
    let result : [Res_ToursimChildCategory]?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent([Res_ToursimChildCategory].self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_ToursimChildCategory : Codable {
    let id : String?
