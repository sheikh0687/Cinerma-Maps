...
struct Api_DetailScheduleTrip : Codable {
    let result : Res_DetailScheduleTrip?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent(Res_DetailScheduleTrip.self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_DetailScheduleTrip : Codable {
    let id : String?
    let user_id : String?
