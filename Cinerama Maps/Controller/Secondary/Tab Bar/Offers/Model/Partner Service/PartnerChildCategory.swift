...
struct Api_PartnerChildCategory : Codable {
    let result : [Res_PartnerChildCategory]?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent([Res_PartnerChildCategory].self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_PartnerChildCategory : Codable {
    let id : String?
