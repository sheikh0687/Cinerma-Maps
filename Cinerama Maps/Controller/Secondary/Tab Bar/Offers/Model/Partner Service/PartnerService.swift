...
struct Api_PartnerService : Codable {
    let result : [Res_PartnerService]?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent([Res_PartnerService].self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_PartnerService : Codable {
    let id : String?
    let cat_id : String?
