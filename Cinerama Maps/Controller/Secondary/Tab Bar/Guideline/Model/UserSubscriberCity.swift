//
//  UserSubscriberCity.swift
//  Cinerama Maps
//
//  Created by Techimmense Software Solutions on 02/08/25.
//

import Foundation

struct Api_UserSubscriberCity : Codable {
    let result : [Res_UserSubscriberCity]?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent([Res_UserSubscriberCity].self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_UserSubscriberCity : Codable {
    let id : String?
